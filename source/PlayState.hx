	...
	override public function create()
	{
			...
			case 'apolstage':
				{
						defaultCamZoom = 0.8;
						curStage = 'apolstage';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('apol/bg'));
						bg.antialiasing = true;
				...
				case 'apolhell':
					{
							defaultCamZoom = 0.8;
							curStage = 'apolhell';
							var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('apol/apolhell'));
							bg.antialiasing = true;
							bg.scrollFactor.set(1, 1);
							bg.active = false;
							add(bg);
		...
		switch (curStage)
		{
			...
			case 'apolstage':
				if(FlxG.save.data.distractions){
					// trailArea.scrollFactor.set();
					var deeztrail = new FlxTrail(dad, null, 4, 24, 0.3, 0.069);
					// deeztrail.changeValuesEnabled(false, false, false, false);
					// deeztrail.changeGraphic()
					add(deeztrail);
					// deeztrail.scrollFactor.set(1.1, 1.1);
					}
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				...
				switch (dad.curCharacter)
				{
					...
					case 'apol':
						...
						camFollow.x = dad.getMidpoint().x + 258;
				}
			...
			if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100)
			{
				...
				#end
				camFollow.setPosition(boyfriend.x + boyfriend.frameWidth/2 - 100 + offsetX, boyfriend.y + boyfriend.height - boyfriend.frameHeight/2 - 150 + offsetY);
